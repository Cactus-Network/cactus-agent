...
import {float, uint32, uint64, uint8} from "../../cactus/types/_python_types_";
import {Chain} from "../../cactus/timelord/types";
import {VDFInfo, VDFProof} from "../../cactus/types/blockchain_format/vdf";
import {bytes32} from "../../cactus/types/blockchain_format/sized_bytes";

export const cactus_timelord_service = "cactus_timelord";
export type cactus_timelord_service = typeof cactus_timelord_service;
...
export type WsFinishedPlotMessage = GetMessageType<cactus_timelord_service, finished_pot_command, TFinishedPotBroadCast>;
export async function on_finished_pot(daemon: TDaemon, callback: (e: WsFinishedPlotMessage) => unknown){
  ...
  const messageListener = (e: WsTimelordMessage) => {
    if(e.origin === cactus_timelord_service && e.command === finished_pot_command){
      callback(e);
    }
  };
  return daemon.addMessageListener(cactus_timelord_service, messageListener);
}
...
export type WsNewCompactProofMessage = GetMessageType<cactus_timelord_service, new_compact_proof_command, TNewCompactProofBroadCast>;
export async function on_new_compact_proof(daemon: TDaemon, callback: (e: WsNewCompactProofMessage) => unknown){
  await daemon.subscribe(metrics_service);
  const messageListener = (e: WsTimelordMessage) => {
    if(e.origin === cactus_timelord_service && e.command === new_compact_proof_command){
      callback(e);
    }
  };
  return daemon.addMessageListener(cactus_timelord_service, messageListener);
...
export type WsSkippingPeakMessage = GetMessageType<cactus_timelord_service, skipping_peak_command, TSkippingPeakBroadCast>;
export async function on_skipping_peak(daemon: TDaemon, callback: (e: WsSkippingPeakMessage) => unknown){
  await daemon.subscribe(metrics_service);
  const messageListener = (e: WsTimelordMessage) => {
    if(e.origin === cactus_timelord_service && e.command === skipping_peak_command){
      callback(e);
    }
  };
  return daemon.addMessageListener(cactus_timelord_service, messageListener);
...
export type WsNewPeakMessage = GetMessageType<cactus_timelord_service, new_peak_command, TNewPeakBroadCast>;
export async function on_new_peak(daemon: TDaemon, callback: (e: WsNewPeakMessage) => unknown){
  await daemon.subscribe(metrics_service);
  const messageListener = (e: WsTimelordMessage) => {
    if(e.origin === cactus_timelord_service && e.command === new_peak_command){
      callback(e);
    }
  };
  return daemon.addMessageListener(cactus_timelord_service, messageListener);
